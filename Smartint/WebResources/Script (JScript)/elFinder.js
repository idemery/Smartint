...
window.elFinder = function(node, opts) {
	 ...
	 * @default 400
	 **/
	height = 400, beeper = $(document.createElement('audio')).hide().appendTo('body')[0], syncInterval, open = function(data) {
		if (data.init) {
			// init - reset cache
			files = {};
		} else {
			...
			for (var i in files) {
				if (files.hasOwnProperty(i) && files[i].mime != 'directory' && files[i].phash == cwd && $.inArray(i, remember) === -1) {
					delete files[i];
				}
			}
		}
	...
	cache = function(data) {
		...
		while (l--) {
			...
			if (f.name && f.hash && f.mime) {
				if (!f.phash) {
					var name = 'volume_' + f.name, i18 = self.i18n(name);
	...
	execShortcut = function(e) {
		var code = e.keyCode, ctrlKey = !!(e.ctrlKey || e.metaKey);
	...
	i18n = this.lang == 'en' ? this.i18['en'] : $.extend(true, {}, this.i18['en'], this.i18[this.lang]);
	...
	this.viewType = this.storage('view') || this.options.defaultView || 'icons', this.sortType = this.storage('sortType') || this.options.sortType || 'name';
	...
	this.draggable = {
		...
		helper : function(e, ui) {
			var element = this.id ? $(this) : $(this).parents('[id]:first'), helper = $('<div class="elfinder-drag-helper"><span class="elfinder-drag-helper-icon-plus"/></div>'), icon = function(mime) {
			...
			hashes = element.is('.' + self.res('class', 'cwdfile')) ? self.selected() : [self.navId2Hash(element.attr('id'))];
	...
	this.droppable = {
		...
		drop : function(e, ui) {
			var dst = $(this), targets = $.map(ui.helper.data('files') || [], function(h) {
				return h || null
			}), result = [], c = 'class', cnt, hash, i, h;
	...
	this.path = function(hash, i18) {
		return files[hash] && files[hash].path ? files[hash].path : this.path2array(hash, i18).join(cwdOptions.separator);
	}
	/**
	...
	this.tmb = function(hash) {
		var file = files[hash], url = file && file.tmb && file.tmb != 1 ? cwdOptions['tmbUrl'] + file.tmb : '';
	...
	this.request = function(options) {
		...
		var smartint = new dropbox_smartint();
		//smartint.request(options).fail(error).donee(success);
		smartint.request(options, success, error);
	...
	this.diff = function(incoming) {
		var raw = {}, added = [], removed = [], changed = [], isChanged = function(hash) {
	 ...
	 */
	this.sync = function() {
		var self = this, dfrd = $.Deferred().done(function() {
			self.trigger('sync');
		}), opts1 = {
			data : {
				cmd : 'open',
			...
			data : {
				cmd : 'tree',
				target : (cwd == this.root()) ? cwd : this.file(cwd).phash
			},
			preventDefault : true
		};

		$.when(this.request(opts1), this.request(opts2)).fail(function(error) {
			dfrd.reject(error);
			error && self.request({
				data : {
					cmd : 'open',
	...
	this.unbind = function(event, callback) {
		var l = listeners[('' + event).toLowerCase()] || [], i = l.indexOf(callback);
	...
	this.trigger = function(event, data) {
		var event = event.toLowerCase(), handlers = listeners[event] || [], i, j;
		...
		if (handlers.length) {
			...
			for ( i = 0; i < handlers.length; i++) {
				...
				try {
					if (handlers[i](event, this) === false || event.isDefaultPrevented()) {
						this.debug('event-stoped', event.type);
						break;
					}
				} catch (ex) {
					window.console && window.console.log && window.console.log(ex.message);
	...
	this.shortcut = function(s) {
		...
		if (this.options.allowShortcuts && s.pattern && $.isFunction(s.callback)) {
			...
			for ( i = 0; i < patterns.length; i++) {
				pattern = patterns[i]
				parts = pattern.split('+');
				code = ( code = parts.pop()).length == 1 ? code > 0 ? code : code.charCodeAt(0) : $.ui.keyCode[code];
	...
	this.clipboard = function(hashes, cut) {
		...
		if (hashes !==
		void (0)) {
			clipboard.length && this.trigger('unlockfiles', {
				files : map()
			});
			remember = [];
			...
			this.trigger('changeclipboard', {
				clipboard : clipboard.slice(0, clipboard.length)
			});
			cut && this.trigger('lockfiles', {
				files : map()
			});
		}
	...
	this.exec = function(cmd, files, opts) {
		return this._commands[cmd] && this.isCommandEnabled(cmd) ? this._commands[cmd].exec(files, opts) : $.Deferred().reject('No such command');
	}
	/**
	...
	this.command = function(name) {
		return name ===
		void (0) ? this._commands : this._commands[name];
	}
	/**
	...
	this.xhrUpload = true; //typeof XMLHttpRequestUpload != 'undefined' && typeof File != 'undefined' && typeof FormData != 'undefined';
	...
	this.error = function() {
		var arg = arguments[0];
		return arguments.length == 1 && typeof (arg) == 'function' ? self.bind('error', arg) : self.trigger('error', {
			error : arg
		});
	}
	...
	this.enable(function() {
		...
		var removed = e.data.removed || [], l = removed.length, rm = function(hash) {
			var file = files[hash];
			if (file) {
	...
	// prepare node
	node.addClass(this.cssClass).bind(mousedown, function() {
		!enabled && self.enable();
	});
	...
	// make node resizable
	this.options.resizable && $.fn.resizable && node.resizable({
		handles : 'se',
		minWidth : 300,
		minHeight : 200
	});
...
elFinder.prototype = {
	...
	rules : {
		defaults : function(data) {
			if (!data || (data.added && !$.isArray(data.added)) || (data.removed && !$.isArray(data.removed)) || (data.changed && !$.isArray(data.changed))) {
				return false;
			}
			return true;
		},
	...
	uploads : {
		...
		iframe : function(data, fm) {
			var self = fm ? fm : this, input = data.input, dfrd = $.Deferred().fail(function(error) {
            ...
            smartint = new dropbox_smartint(),
            parentPath = smartint.decryptPath(self.cwd().hash),
            iframeUploadUrl = smartint.getUploadUrlForIframe(parentPath, input),
            //self.uploadURL



            //method="post" enctype="multipart/form-data" encoding="multipart/form-data"
            form = $('<form action="' + iframeUploadUrl + '" method="PUT" enctype="image/png" target="' + name + '" style="display:none"><input type="hidden" name="cmd" value="upload" /></form>'), msie = $.browser.msie,
			...
			}, iframe = $('<iframe src="' + ( msie ? 'javascript:false;' : 'about:blank') + '" name="' + name + '" style="position:absolute;left:-1000px;top:-1000px" />').bind('load', function() {
				iframe.unbind('load').bind('load', function() {
				    var data = self.parseUploadData(iframe.contents().text());
				    if (data === "")
				        dfrd.resolve(data);

					var response = smartint.convert_dropbox_response_of_uploaded_file(data);
					onload();
			...
			form.append('<input type="hidden" name="' + (self.newAPI ? 'target' : 'current') + '" value="' + self.cwd().hash + '"/>').append('<input type="hidden" name="html" value="1"/>').append($(input).attr('name', 'upload[]'));
		...
		xhr : function(data, fm) {
			var self = fm ? fm : this, dfrd = $.Deferred().fail(function(error) {
				error && self.error(error);
			}).done(function(data) {
				data.warning && self.error(data.warning);
				data.removed && self.remove(data);
			...
			var totalFilesSize = 0, totalBytesLoaded = 0, i, j;
			...
			xhr.upload.addEventListener('progress', function(e) {
				...
				if (e.lengthComputable) {
					...
					curr = parseInt((e.loaded + totalBytesLoaded) * 100 / totalFilesSize);
					
					if (parseInt(e.loaded * 100 / e.total) == 100) totalBytesLoaded += e.loaded;
					
					console.log("current:" + curr);
					console.log("Total:" + totalFilesSize);
			...
			var parentPath = smartint.decryptPath(self.cwd().hash),
				response = {
					added: []
				};
			smartint.uploadFile(self.options, smartint, xhr, dfrd, parentPath, files, 0, response, smartint.uploadFile);
	...
	one : function(event, callback) {
		var self = this, h = $.proxy(callback, function(event) {
			setTimeout(function() {
	...
	_sortRules : {
		...
		date : function(file1, file2) {
			var date1 = file1.ts || file1.date, date2 = file2.ts || file2.date;
	...
	compare : function(file1, file2) {
		var self = this, type = self.sortType, asc = self.sortOrder == 'asc', stick = self.sortStickFolders, rules = self.sortRules, sort = rules[type], d1 = file1.mime == 'directory', d2 = file2.mime == 'directory', res;
	...
	notify : function(opts) {
		...
		if (cnt > 0) {
			...
			if (progress < 100 && ( total = notify.data('total')) >= 0 && ( prc = notify.data('progress')) >= 0) {
				...
				ndialog.find('.elfinder-notify-progress').animate({
	...
	confirm : function(opts) {
		...
		}, apply = this.i18n('apllyAll'), label, checkbox;
		...
		if (opts.all) {
			...
			options.open = function() {
				var pane = $(this).next(), width = parseInt(pane.children(':first').outerWidth() + pane.children(':last').outerWidth());
	...
	mime2kind : function(f) {
		...
		var mime = typeof (f) == 'object' ? f.mime : f, kind = this.kinds[mime] || 'unknown';
	...
	formatDate : function(file, ts) {
		var self = this, ts = ts || file.ts, i18 = self.i18, date, format, output, d, dw, m, y, h, g, i, s;
